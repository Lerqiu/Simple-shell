...
#ifdef STUDENT
    int relativePLen = strlen(argv[0]);
    char *relativeP = argv[0];
    int pathLen = strlen(path);
    int offset = 0;

    while (offset < pathLen) {
      int rootPLen = 0;
      char *rootP = (char *)(path + offset);
      while (rootP[rootPLen] != ':' && rootP[rootPLen] != '\0')
        rootPLen++;

      offset += rootPLen + 1; // We don't need ':'
      rootP[rootPLen] = '\0';
      char *absolutePath = malloc(relativePLen + rootPLen + 3);
      sprintf(absolutePath, "%s/%s", rootP, relativeP);

      argv[0] = absolutePath;
      (void)execve(absolutePath, argv, environ);
      free(absolutePath);
      rootP += rootPLen;
    }
#endif /* !STUDENT */
